...
def _minute_index(sentence: dict) -> int:
    """Which one-minute bucket a sentence belongs to, by when speech begins."""
    t = sentence.get("speech_start", sentence.get("start_time", 0.0))
...
def build_scene_segments_from_sentences(
    ...
    Deterministic only (no LLM) so long narrations stay reliable:
    • Sentences bucket by the minute they *start* speaking.
    • Each minute is split into its target scene count via time-balanced groups
      (never splitting a sentence).
    • Scene start/end = first/last sentence word timestamps in the group.
    """
    del title  # kept for API compatibility; grouping is fully deterministic.
    ...
    logger.info(
        "Scene grouping: %s sentences → %s scenes across %s minute bucket(s).",
        len(sents), len(segments), len(plan),
    )
    return segments
...
def split_and_prompt(
    ...
    require_stt_segments: bool = False,
) -> list[dict]:
    """
    Two-stage scene build:

    1. Use pre-built scene segments (sentence-aligned to the real voice-over via
       build_scene_segments_from_sentences) when provided. When
       ``require_stt_segments`` is True (voice-over projects), missing segments
       is a hard error — no equal-word fallback.
    ...
    if pre_segments is None:
        if require_stt_segments:
            from engine.transcribe import STTError, STT_USER_MESSAGE
            raise STTError(
                "Sentence timeline is required for voice-over projects. "
                + STT_USER_MESSAGE
            )
...
SAFETY_REPHRASE_SYSTEM = """You revise image-generation prompts that were rejected by an AI safety/moderation system.

Return JSON with keys:
  "prompt": string — full revised positive prompt (at least ~80 words), same Tatterveil atmospheric photorealistic documentary look.
  "negative_prompt": string or null — revised negative constraints.

Rules:
- Keep the same scene intent, historical period, composition, and documentary tone.
- Remove or soften wording that could trigger violence, gore, weapons in action, explicit harm, disturbing medical imagery, or other sensitive content.
- Replace blocked elements with safer visual alternatives: silhouettes, distant figures, aftermath without gore, symbolic objects, atmospheric mood, implied action off-frame.
- Maintain 16:9 landscape photorealistic language; no watermarks; no text in image.
- Do NOT change the setting era unless required for safety.
"""


def rephrase_prompt_for_safety(
    prompt: str,
    negative_prompt: str | None,
    script_segment: str,
    attempt_index: int,
) -> tuple[str, str | None]:
    """Ask the text model to soften a blocked image prompt for another try."""
    client = _get_client()
    payload = {
        "blocked_prompt": prompt,
        "blocked_negative_prompt": negative_prompt or "",
        "script_segment": script_segment,
        "rephrase_attempt": attempt_index + 1,
        "instruction": (
            "The image API rejected the prompt for safety. Produce a safer variant "
            "that still depicts the same documentary scene."
        ),
    }
    for attempt in range(config.MAX_RETRIES):
        try:
            ...
            if isinstance(neg, str):
                ...
                neg = None
            if not pos:
                raise ValueError("empty prompt from safety rephrase model")
            return pos, neg
        except Exception as exc:
            if is_moderation_error(exc):
            ...
            logger.warning("Safety rephrase attempt %s failed: %s", attempt + 1, exc)
            if attempt < config.MAX_RETRIES - 1:
                time.sleep(config.RETRY_DELAY * (attempt + 1))
...
def generate_image(
    ...
    Returns (output_path, elapsed_seconds). Retries transient errors with
    exponential back-off. On safety blocks during initial generation, auto-
    rephrases the prompt up to MODERATION_REPHRASE_ATTEMPTS times.
    """
    images_dir = project_dir / "images"
    ...
    max_rephrase = max(1, int(config.MODERATION_REPHRASE_ATTEMPTS))

    for rephrase_round in range(max_rephrase):
        for attempt in range(config.MAX_RETRIES):
            try:
                ...
                if is_moderation_error(exc):
                    logger.warning(
                        "Scene %s image blocked (rephrase %s/%s): %s",
                        scene_num, rephrase_round + 1, max_rephrase, exc,
                    )
                    break
                logger.warning(
        ...
        if rephrase_round < max_rephrase - 1:
            pos_prompt, neg_prompt = rephrase_prompt_for_safety(
                prompt=pos_prompt,
                negative_prompt=neg_prompt if isinstance(neg_prompt, str) else None,
                script_segment=str(scene.get("script_segment") or ""),
                attempt_index=rephrase_round,
            )
            scene = dict(scene)
            scene["prompt"] = pos_prompt
            scene["negative_prompt"] = neg_prompt
            prompt = _build_final_prompt(scene)
            logger.info(
                "Scene %s: retrying image with safety-rephrased prompt (%s/%s).",
                scene_num, rephrase_round + 2, max_rephrase,
            )
            continue

        logger.error(
            "Scene %s image blocked by safety system after %s rephrase attempt(s).",
            scene_num, max_rephrase,
        )
        raise ContentModerationError(
            MODERATION_SCENE_MESSAGE, original=last_exc
        ) from last_exc
