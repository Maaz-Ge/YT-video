...
# Also cap each STT slice by duration (seconds) so long narrations are processed
# in manageable windows with overlap at boundaries.
STT_MAX_CHUNK_SECONDS: float = float(os.getenv("STT_MAX_CHUNK_SECONDS", "600"))
STT_CHUNK_OVERLAP_SECONDS: float = float(os.getenv("STT_CHUNK_OVERLAP_SECONDS", "2.0"))
# Max sentence-level rephrase attempts when OpenAI blocks an image for safety.
MODERATION_REPHRASE_ATTEMPTS: int = int(os.getenv("MODERATION_REPHRASE_ATTEMPTS", "3"))
